...
\begin{center}
\begin{huge}
\textbf{\\Ejercicios Econometría Propuesto}
\end{Large}\\
%{\large 2023-2}\\
\end{center}
...
{{2. Discusi\'on: posible omisi\'on de $X_2$}
...
\textbf{3. Regresi\'on Lineal M\'ultiple: $Y \sim X_1 + X_2$}
...
{Producto final: $\hat{\boldsymbol{\beta}} = (X^TX)^{-1}(\mathbf{X}^\top \mathbf{Y})$}
...
\textbf{Cálculo de $R^2$ y $R_{\text{ajustado}}^2$ (modelo múltiple)}
...
Verificaci\'on con software (opcional)
...
\item (4 puntos para certamen) Se tienen los siguientes datos de salario, años de educación y experiencia laboral de 5 individuos:
\begin{center}
...
\begin{enumerate}
    ...
    $$\widehat{Salario} = \hat{\beta_0} + \hat{\beta_1} \cdot Educ$$
    Calcule los valores de los estimadores $\hat{\beta_0}$ y $\hat{\beta_1}$ y SST, SSE, SSR, $R^2$.
    \\


    \item  ¿Qué sesgos puede tener el modelo anterior?
    \\


    \item Luego se realiza un modelo incluyendo la variable Experiencia, por lo que el salario estaría explicado por la educación y la experiencia de la siguiente manera:
    $$\widehat{Salario} = \hat{\beta_0} + \hat{\beta_1} \cdot Educ + \hat{\beta_2} \cdot Exp$$, donde la matriz $(X^TX)^{-1}$ esta dada por:
      \[
(X^TX)^{-1}
= 
\begin{bmatrix}
...
-3.21 & -0.0882 & 0.294
\end{bmatrix}.
\] Calcule los valores de \textbf{${\beta}$}, SST, SSE, SSR, $R^2$ y $R^2$ ajustado.
    \\

    \item  Compare ambos modelos. 
    \\
